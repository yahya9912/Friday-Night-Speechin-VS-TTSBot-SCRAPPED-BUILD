// I borrowed this from the Soulless DX source code, probably won't use it, but if this is used, then whoops. Don't kill me Gray!
// I think I now made this unused, this sucks in my opinion anyway.
function onKeyShit(pressed, justPressed, justReleased) {
	...
	if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && !up && !down && !right && !left) {
		if (StringTools.startsWith(boyfriend.animation.curAnim.name, 'sing')
			&& StringTools.endsWith(!boyfriend.animation.curAnim.name, 'miss')) {
...
function onNoteUpdatePost(note:Note) {
	...
	if (note.mustPress) {
		...
		if (note.strumTime > Conductor.songPosition - safeZoneOffset && note.strumTime < Conductor.songPosition + (safeZoneOffset * 0.5))
			note.canBeHit = true;
		else
			note.canBeHit = false;
